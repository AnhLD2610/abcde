...
class Manager(object):
    def __init__(self, config) -> None:
        super().__init__()
        self.config = config
        # self.fc1 = nn.Linear(1536, 512).to(config.device)
        # self.fc2 = nn.Linear(512, 256).to(config.device)
        # self.fc3 = nn.Linear(256, 128).to(config.device)
        # self.fc4 = nn.Linear(128, 1).to(config.device)
    ...
    def train_model(self, encoder, training_data, seen_des, is_memory=False):
        ...
        for i in range(epoch):
            for batch_num, (instance, labels, ind) in enumerate(data_loader):
                ...
                #         score = F.relu(self.fc3(score))
                #         score = torch.sigmoid(self.fc4(score))
                ...
                # loss = 1*loss + 2*loss2 + 0.5*loss_online + 1*loss3 + 1*loss_cross_encoder
                # loss = loss_cross_encoder
    ...
    def eval_encoder_proto_des_cross_encoder(self, encoder, seen_proto, seen_relid, test_data, rep_des):
        """
        Args:
            encoder: Encoder
        ...
        # Move seen relation descriptions to the correct device
        rep_des = rep_des.to(self.config.device)

        for batch_num, (instance, label, _) in enumerate(test_loader):
            for k in instance.keys():
                instance[k] = instance[k].to(self.config.device)
            
            # Forward pass through the encoder to get the test instance representations
            hidden = encoder(instance)
            fea = hidden  # No need to move to CPU if cross-encoder happens on GPU
            
            num_prototypes = rep_des.size(0)
            
            # Prepare logits list
            logits = []
            
            for i6 in range(fea.size(0)):  # Iterate through each instance in the batch
                x = fea[i6]  # Shape: (768)
                scores = []
                for j6 in range(num_prototypes):  # Iterate through all relation descriptions
                    des = rep_des[j6]  # Shape: (768)
                    
                    # Concatenate the test instance and the description (shape: 1536)
                    concat_input = torch.cat((x, des), dim=0)  # Shape: (1536)
                    
                    # Forward pass through the fully connected layers
                    out = F.relu(self.fc1(concat_input))
                    out = F.relu(self.fc2(out))
                    out = F.relu(self.fc3(out))
                    score = torch.sigmoid(self.fc4(out))                    
                    # Append the score to the scores list for this instance
                    scores.append(score)
                    # print(score)
                
                # Stack the scores and append to logits (shape: (num_prototypes,))
                scores = torch.stack(scores).squeeze()  # Shape: (num_prototypes,)
                # print(scores)
                # print(scores.shape) # shape torch.Size([6])

                logits.append(scores)
                # print(logits.shape)
            
            # Stack all logits (shape: (batch_size, num_prototypes))
            logits = torch.stack(logits)  # Shape: (batch_size, num_prototypes)
            
            # Get the predicted relation by argmax over the logits
            cur_index = torch.argmax(logits, dim=1)  # Shape: (batch_size)
            pred = []
            
            # Map predicted indices to the actual relation ids
            for i7 in range(cur_index.size(0)):
                pred.append(seen_relid[int(cur_index[i7])])
            pred = torch.tensor(pred)
            # Calculate accuracy
            correct = torch.eq(pred, label).sum().item()
            acc = correct / batch_size
            ...
            # Print batch accuracy and total accuracy
            sys.stdout.write('[EVAL] batch: {0:4} | acc: {1:3.2f}%,  total acc: {2:3.2f}%   ' \
                            .format(batch_num, 100 * acc, 100 * (corrects / total)) + '\r')
            sys.stdout.flush()
...
#         # Get number of prototypes and their dimensionality
#         num_prototypes = rep_des.size(0)
        
#         # Prepare logits list
#         logits = []
        
#         # For each instance in the batch, concatenate with all relation descriptions
#         for i in range(fea.size(0)):  # Iterate through each instance in the batch
#             x = fea[i]  # Shape: (768)
#             scores = []
#             for j in range(num_prototypes):  # Iterate through all relation descriptions
#                 des = rep_des[j]  # Shape: (768)
                
#                 # Concatenate the test instance and the description (shape: 1536)
#                 concat_input = torch.cat((x, des), dim=-1)  # Shape: (1536)
                
#                 # Forward pass through the fully connected layers
#                 out = F.relu(self.fc1(concat_input))
#                 out = F.relu(self.fc2(out))
#                 out = F.relu(self.fc3(out))
#                 score = self.fc4(out)  # Shape: (1)
...
#         return corrects / total
    ...
    def train(self):
        ...
        for step, (training_data, valid_data, test_data, current_relations, \
            ...
            # ac1,ac1_des, ac1_rrf = self.eval_encoder_proto_des_cross_encoder(encoder,seen_proto,seen_relid,test_data_initialize_cur,rep_des)
            # ac2,ac2_des, ac2_rrf = self.eval_encoder_proto_des_cross_encoder(encoder,seen_proto,seen_relid,test_data_initialize_seen,rep_des)
            ac1= self.eval_encoder_proto_des_cross_encoder(encoder,seen_proto,seen_relid,test_data_initialize_cur,rep_des)
            ac2= self.eval_encoder_proto_des_cross_encoder(encoder,seen_proto,seen_relid,test_data_initialize_seen,rep_des)
