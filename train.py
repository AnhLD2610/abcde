    ...
    def train_model(self, encoder, training_data, seen_des, is_memory=False):
        ...
        for i in range(epoch):
            for batch_num, (instance, labels, ind) in enumerate(data_loader):
                ...
                for k in instance.keys():
                    instance[k] = instance[k].to(self.config.device)
                # for k in range(self.num_gen_augment):
                ...
                repeated_hidden = torch.repeat_interleave(hidden, repeats=3, dim=0)
                ...
                for idx in range(len(new_labels)):
                    ...
                    loss_online.append(online_contrastive_loss(rep_des, repeated_hidden, label_for_loss))
                ...
                loss = 1*loss + 1*loss2 + 1*loss_online + 1*loss3 
    ...
    def get_augment_data_label(self, instance, labels):
        ...
        for i in range(batch_size):
            for j in range(batch_size):
                ids1 = instance[i][instance[i] != 0]
                ids2 = instance[j]

                combined = torch.cat((ids1, ids2))

                if len(combined) > max_len - 1:
                    combined = combined[:max_len - 1]
                    combined = torch.cat((combined, torch.tensor([102])))

                if len(combined) < max_len:
                    padding = torch.zeros(max_len - len(combined), dtype=torch.float)
                    combined = torch.cat((combined, padding))

                augmented_data.append(combined)

                new_label = torch.tensor([labels[i, 0], labels[j, 1]])
                augmented_labels.append(new_label)

        # Convert the lists of pairs into tensors
